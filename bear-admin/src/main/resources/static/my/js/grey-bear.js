        ...
        table: {
            ...
            init: function(options) {
                ...
                $('#bootstrap-table').bootstrapTable({
                    ...
                    search: false,                                      // 是否显示搜索框功能 $.common.visible(options.search)
                    ...
                    showExport: false,                                  // 是否支持导出文件 $.common.visible(options.showExport)
            ...
            queryParams: function(params) {
                ...
                var temp = {//如果是在服务器端实现分页，limit、offset这两个参数是必须的
                    pageNo : params.offset / params.limit + 1,//从数据库第几条记录开始
                    pageSize : params.limit,
                    sortProperties: params.sort,      //排序列名
                    direction: params.order //排位命令（desc，asc）
                };
                return temp;
            },
            // 请求获取数据后处理回调函数
            ...
            search: function(formId) {
                ...
                params.queryParams = function(params) {
                    var queryParam = {};
                    $('#page-form').find('[name]').each(function () {
                        var value = $(this).val();
                        if (value != '') {
                            queryParam[$(this).attr('name')] = value;
                        }
                    });
            ...
            addQueryParam:function (columns) {
              if(null != columns && columns.length>0){
                  $("#page-form").parent().show();
                  $.each(columns,function (index,obj) {
                      $('#page-form div ul').prepend(" <li>"+obj.label+"：<input type=\"text\" name=\""+obj.name+"\" placeholder=\""+obj.tips+"\"/></li>");
                  });
              }else{
                  $("#page-form").parent().hide();
              }
            }
        },
        // 表格树封装处理
